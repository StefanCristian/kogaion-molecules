...
if [ -z "${BUILDING_DAILY}" ]; then
        ...
        sed -i "/^officialrepositoryid/ s/rogentoslinux.org/rogentos-weekly/" "${repo_conf}" || exit 1
        sed -i "/^official-repository-id/ s/rogentoslinux.org/rogentos-weekly/" "${repo_conf}" || exit 1
        sed -i "/^repository =/ s/rogentoslinux.org/rogentos-weekly/" "${repo_conf}" || exit 1
