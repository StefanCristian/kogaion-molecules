...
if [ -z "${BUILDING_DAILY}" ]; then
	...
	#sed -i "/^officialrepositoryid/ s/kogaionlinux/kogaion-weekly/" "${repo_conf}" || exit 1
	#sed -i "/^official-repository-id/ s/kogaionlinux/kogaion-weekly/" "${repo_conf}" || exit 1
	#sed -i "/^repository =/ s/kogaionlinux/kogaion-weekly/" "${repo_conf}" || exit 1

	sed -i "/^officialrepositoryid/ s/kogaion-weekly/kogaionlinux/" "${repo_conf}" || exit 1
	sed -i "/^official-repository-id/ s/kogaion-weekly/kogaionlinux/" "${repo_conf}" || exit 1
	sed -i "/^repository =/ s/kogaion-weekly/kogaionlinux/" "${repo_conf}" || exit 1
	...
	dst_conf="${repo_conf_d}/entropy_kogaionlinux"
	if [ -f "${src_conf}" ]; then
		mv "${src_conf}" "${dst_conf}" || exit 1
		#sed -i "/^\[kogaionlinux\]$/ s/kogaionlinux/kogaion-weekly/" \
